#include "EmonLib.h"
#include "config.h" // WIFI_SSID, WIFI_PASS, NODE_SERVER_URL, APPS_SCRIPT_URL
#include <HTTPClient.h>
#include <WiFi.h>
#include <WiFiClientSecure.h>

// ── Calibración sensores ─────────────────────────────────────
#define vCalibration    151
#define currCalibration 0.06
#define NUM_MUESTRAS    5
#define WARMUP_MS       30000  // τ ≈ 22s medido experimentalmente

EnergyMonitor emon;
float kWh = 0;
unsigned long lastMillis = 0;

// ════════════════════════════════════════════════════════════
//  1. Envío HTTPS al Apps Script
// ════════════════════════════════════════════════════════════
void sendToAppsScript(float vrms, float irms, float power) {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[WiFi] Desconectado — reintentando...");
    WiFi.reconnect();
    return;
  }

  char json[200];
  sprintf(json, "{\"vrms\":%.2f,\"irms\":%.4f,\"power\":%.4f,\"kWh\":%.6f}",
          vrms, irms, power, kWh);

  WiFiClientSecure client;
  client.setInsecure();

  HTTPClient http;
  http.begin(client, APPS_SCRIPT_URL);
  http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
  http.addHeader("Content-Type", "application/json");
  http.setTimeout(10000);

  unsigned long t0 = millis();
  int code = http.POST(json);
  unsigned long dt = millis() - t0;

  if (code == 200 || code == 302) {
    Serial.printf("[OK]  Apps Script respondió HTTP %d  (%lu ms)\n", code, dt);
  } else {
    Serial.printf("[ERR] HTTP %d  (%lu ms)\n", code, dt);
  }

  http.end();
}

// ════════════════════════════════════════════════════════════
//  2. Envío HTTP al servidor local
// ════════════════════════════════════════════════════════════
void sendToLocalServer(float vrms, float irms, float power) {
  if (WiFi.status() != WL_CONNECTED)
    return;

  char json[220];
  sprintf(json,
          "{\"vrms\":%.2f,\"irms\":%.4f,\"power\":%.4f,\"kWh\":%.6f,\"uptime\":%lu}",
          vrms, irms, power, kWh, millis());

  WiFiClient client;
  HTTPClient http;
  http.begin(client, NODE_SERVER_URL);
  http.addHeader("Content-Type", "application/json");
  http.setTimeout(3000);

  int code = http.POST(json);
  if (code > 0) {
    Serial.printf("[Local] HTTP %d\n", code);
  } else {
    Serial.printf("[Local] ERR %d — edita .env y ejecuta generar_config.py\n", code);
  }
  http.end();
}

// ════════════════════════════════════════════════════════════
//  Lectura + promedio + cálculo de energía
// ════════════════════════════════════════════════════════════
void myTimerEvent() {
  float sumVrms = 0, sumIrms = 0, sumPower = 0;

  for (int i = 0; i < NUM_MUESTRAS; i++) {
    emon.calcVI(20, 2000);
    sumVrms  += emon.Vrms;
    sumIrms  += emon.Irms;
    sumPower += emon.apparentPower;
  }

  float vrms  = sumVrms  / NUM_MUESTRAS;
  float irms  = sumIrms  / NUM_MUESTRAS;
  float power = sumPower / NUM_MUESTRAS;

  unsigned long now = millis();
  kWh += power * (now - lastMillis) / 3600000000.0;
  lastMillis = now;

  Serial.printf("Vrms: %.2fV  Irms: %.4fA  Power: %.4fW  kWh: %.6f\n",
                vrms, irms, power, kWh);

  sendToLocalServer(vrms, irms, power);
  sendToAppsScript(vrms, irms, power);
}

// ════════════════════════════════════════════════════════════
void setup() {
  Serial.begin(9600);

  emon.voltage(35, vCalibration, 1.7);
  emon.current(34, currCalibration);

  WiFi.begin(WIFI_SSID, WIFI_PASS);
  WiFi.setAutoReconnect(true);
  Serial.print("Conectando WiFi");
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
  Serial.println("\nConectado: " + WiFi.localIP().toString());

  // ── Warm-up: descarga el condensador del ZMPT101B ────────
  // τ ≈ 22s calculado a partir de los datos experimentales.
  // 30s lleva la carga residual al ~26% del valor inicial.
  // Si persisten valores grandes al inicio, sube a 45000.
  Serial.println("Estabilizando sensores (30s)...");
  unsigned long t0 = millis();
  while (millis() - t0 < WARMUP_MS) {
    emon.calcVI(20, 2000);
    delay(100);
  }
  Serial.println("Listo.");
  // ─────────────────────────────────────────────────────────

  lastMillis = millis(); // reiniciar DESPUÉS del warm-up
}

// ════════════════════════════════════════════════════════════
void loop() {
  static unsigned long lastEvent = 0;
  const unsigned long INTERVALO = 5000;

  if (millis() - lastEvent >= INTERVALO) {
    lastEvent = millis();
    myTimerEvent();
  }
}